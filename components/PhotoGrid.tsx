  ...
  if (photos.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        ...
        <p className="text-muted-foreground text-sm">{noPhotos}</p>
      </div>
    );
  }
      ...
      {photos.map((photo) => (
        <div key={photo.filename} className="break-inside-avoid mb-4">
          ...
          <img
            ...
            loading="lazy"
            decoding="async"
            className="w-full h-auto rounded-xl border border-border transition-transform duration-300 hover:scale-[1.01]"
          />
        </div>
      ))}
    </div>
